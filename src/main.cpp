    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2')
    {
      ...
      if (s != "")
      {
        ...
        if (event == "telemetry")
        {
          ...
          *
          */
          Eigen::VectorXd ptsxE = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(
              ptsx.data(), ptsx.size());
          Eigen::VectorXd ptsyE = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(
              ptsy.data(), ptsy.size());
          ...
          {
            auto curr_x = poly_inc * i;
            next_x_vals.push_back(curr_x);
            next_y_vals.push_back(polyeval(coeffs, curr_x));
          }
